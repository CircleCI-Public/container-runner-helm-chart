...
![Version: 101.1.11](https://img.shields.io/badge/Version-101.1.11-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 3.1.12](https://img.shields.io/badge/AppVersion-3.1.12-informational?style=flat-square)
...
| agent.image | object | `{"digest":"","pullPolicy":"Always","registry":"","repository":"circleci/runner-agent","tag":"kubernetes-3.1.12-9831-dca4a16"}` | Agent image settings. NOTE: Setting an image digest will take precedence over the image tag |
...
| logging | object | `{"image":{"registry":"","repository":"circleci/logging-collector","tag":"3.1.12-9831-dca4a16"},"rbac":{"create":true,"role":{"name":"logging-collector","rules":[]}},"serviceAccount":{"annotations":{},"create":true,"name":"logging-collector","secret":{"name":"logging-collector-token"}}}` | Configuration values for the logging containers. These containers run alongside service containers and stream their logs to the CircleCI UI |
...
| logging.image.tag | string | `"3.1.12-9831-dca4a16"` | Image tag for the logging collector. Should match container agent version to ensure compatibility. |
...
| orchestrator | object | `{"image":{"digest":"","registry":"","repository":"","tag":""},"resources":{}}` | Configures the orchestrator init container, which initializes the primary container environment. For more details, see the runner-init project's [README](https://github.com/circleci/runner-init?tab=readme-ov-file#runner-init--goat). |
...
| orchestrator.resources | object | `{}` | Override the default resource limits and requests for the orchestrator init container. These default to 10m CPU / 32Mi memory requests and 50m CPU / 64Mi memory limits. resources:   requests:     cpu: 10m     memory: 32Mi   limits:     cpu: 50m     memory: 64Mi |
